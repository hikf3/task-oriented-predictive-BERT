...
def seq_fn(pts_sls, outFile):
    print("Creating sequence for each PATID")
    dataSize = len(pts_sls)
    np.random.seed(0)
    ind = np.random.permutation(dataSize)
    train_indices = ind
    subset_ptencs_s = [pts_sls[i] for i in train_indices]
    bertencsfile = outFile  + '_train'
    pickle.dump(subset_ptencs_s, open(bertencsfile, 'a+b'), -1)

### Main Function
if __name__ == '__main__':
    diagFile = sys.argv[1]
    print('DX_data', sys.argv[1])
    typeFile = sys.argv[2]
    ...
    # Load the excluded_patient_ids from a CSV file (test dataset)
    excluded_patient_ids_df = pd.read_csv('/Users/hikf3/Documents/GitHub/Doctoral-Research/BEHRT/DATA_PREPROCESSING/ID_DX_test_phe.csv')
    excluded_patient_ids = excluded_patient_ids_df.iloc[:,0].values

    print('length of excluded patids', len(excluded_patient_ids))

    # Filter the dataset to exclude specific patient IDs
    filtered_data_diag = data_diag[~data_diag['patient_sk'].isin(excluded_patient_ids)]
    ...
    if typeFile == 'NA':
        types = {'mask': -1,'empty_pad': 0, 'unk': 1, 'cls': 2, 'sep': 3 }
    else:
        with open(typeFile, 'rb') as t2:
            types = pickle.load(t2)

    n_data = filtered_data_diag
    count = 0
    pts_sls = []
    ...
    for Pt, group in n_data.groupby('patient_sk'):
        ...
        v = 0
        len_dx =[]
        #group['diagnosis']= ['cls'] + group['diagnosis']
        print(Pt)
        print(group['diagnosis'])
        print(group['admit_dt_tm'])
